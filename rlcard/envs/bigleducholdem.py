...
from rlcard.games.bigleducholdem import Game
...
class BigleducholdemEnv(Env):
    ''' Bigleduc Hold'em Environment
    '''

    def __init__(self, config):
        ''' Initialize the Limitholdem environment
        '''
        self.name = 'bigleduc-holdem' 
        self.default_game_config = DEFAULT_GAME_CONFIG
        self.game = Game()
        super().__init__(config)
        self.actions = ['call', 'raise', 'fold', 'check']
        # * 12 + 12 + 2 * (1+2+2*6+4*6) = 102
        self.state_shape = [[102] for _ in range(self.num_players)]
        self.action_shape = [None for _ in range(self.num_players)]

        with open(os.path.join(rlcard.__path__[0], 'games/bigleducholdem/card2index.json'), 'r') as file:
    ...
    def _extract_state(self, state):
        ...
        if public_card:
            obs[self.card2index[public_card]+12] = 1
        obs[state['my_chips']+24] = 1
        obs[sum(state['all_chips'])-state['my_chips']+63] = 1
